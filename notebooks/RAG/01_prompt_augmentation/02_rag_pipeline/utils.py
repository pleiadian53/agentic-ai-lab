...
if model_name:
    model_name = os.path.join(model_name, "BAAI/bge-base-en-v1.5")
else:
...
def generate_with_single_input(prompt: str, 
    ...
    if (not together_api_key) and ('TOGETHER_API_KEY' not in os.environ):
        ...
        json_dict['choices'][-1]['message']['role'] = role_val.name.lower() if hasattr(role_val, 'name') else str(role_val).lower()
    try:
